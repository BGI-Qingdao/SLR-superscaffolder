...
struct  AppConfig
{
    ...
    void Init(const std::string & prefix , float ls , float ss )
    {
        ...
        if ( len_factor <= 0.0f )
            len_factor = 0.01f ;
        else if ( len_factor > 1.0f )
            len_factor = 1.0f ;

        if ( sim_factor<= 0.0f )
            sim_factor = 0.01f ;
        else if ( sim_factor > 1.0f )
            sim_factor = 1.0f ;

        loger<<BGIQD::LOG::lstart()<<" len_threshold_factor used is "<<len_factor<<BGIQD::LOG::lend();
        loger<<BGIQD::LOG::lstart()<<" sim_threshold_factor used is "<<sim_factor<<BGIQD::LOG::lend();
    }
    ...
    void LoadGraph()
    {
        ...
        std::string line ;
        int id  = 0 ;
        auto in = BGIQD::FILES::FileReaderFactory::GenerateReaderFromFileName(fName.connInfo());
        if(in == NULL )
    ...
    void CalculateThreshold()
    {
        ...
        loger<<BGIQD::LOG::lstart()<<" total linear node "<<index<<BGIQD::LOG::lend();
        std::sort(linear_len.begin() , linear_len.end());
        std::sort(linear_sim.rbegin() , linear_sim.rend());
        len_threshold = linear_len[ int(index * len_factor) -1];
        sim_threshold = linear_sim[ int(index * sim_factor) -1];
        loger<<BGIQD::LOG::lstart()<<" used linear len threshold "<<len_threshold<<BGIQD::LOG::lend();
        loger<<BGIQD::LOG::lstart()<<" used linear sim threshold "<<sim_threshold<<BGIQD::LOG::lend();
    }
    void DeleteWeekConn()
    {
        // solve multi
        auto  get_oppo = [this] (unsigned int to  , bool to_order , bool positive) {
            auto & curr_to = edges[to];
            //A1->B1 
            //A2<-B2
        ...
        loger<<BGIQD::LOG::lstart()<<"length-sim delete"<<del_count<<BGIQD::LOG::lend();
        report_freq();
    }

    void ExtractLinearFromGraph()
    {

        auto extractPath = [this]( unsigned int to_s , bool to_order, bool search_order,BGIQD::SOAP2::KeyEdge & curr,BGIQD::SOAP2::ContigRoad & path , bool circle = false)
        {
            ...
            {
                path.real_contig.push_back(edges.at(to).bal_id);
            }
            else
            {
                path.real_contig.push_back(edges.at(to).edge_id);
            }
            bool order = search_order;//
            bool torder = to_order; // 
            if( edges.at(to).IsMarked())
                return ;
            while( (! edges.at(to).IsCircle())&& edges.at(to).IsLinear() && !edges.at(to).IsMarked() && to != root && to != root_bal)
            {
                auto & curr_to = edges.at(to);
                curr_to.Mark();
                //downstream
                if( order )
                {
            ...
            if( ! circle && edges.at(to).IsMarked() )
                return ;
            // line->next_k->
            // <-next_k<-line
            path.tailin = false;
    ...
    void PrintContigRoad()
    {
        // print linear collection
        auto outf = BGIQD::FILES::FileWriterFactory::GenerateWriterFromFileName(fName.contigroad());
        if( outf == NULL )
        ...
        loger<<BGIQD::LOG::lstart()<<"len freq"<< '\n'<<len_freq.ToString() << BGIQD::LOG::lend();
    }

    float len_factor , sim_factor ;
    float sim_threshold ;
    int len_threshold ;
}config;
...
int main(int argc , char ** argv)
{
    ...
    config.Init(prefix.to_string() ,len_factor.to_float() ,sim_factor.to_float());

    config.LoadGraph();

    config.CalculateThreshold();

    config.DeleteWeekConn();

    config.ExtractLinearFromGraph() ;

    config.PrintContigRoad() ;
    return 0 ;
}
