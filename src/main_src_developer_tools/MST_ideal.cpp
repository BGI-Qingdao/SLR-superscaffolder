...
struct AppConf
{
    ...
    void Init(const std::string & prefix, int min_c , float m53 , float min_js )
    {
        ...
        BGIQD::LOG::logfilter::singleton().get("MST_ideal", BGIQD::LOG::loglevel::INFO,lger);
    }
    ...
    void MST()
    {
        ...
        {
            pair.second.CorrectGraph(unique_contigs);
        }
    }
    //Done
    ...
    std::set<unsigned int> unique_contigs;
    void LoadContigType( const std::string & file  )
    {
        ...
        while(!std::getline(*in,line).eof()) {
            if( line == "id\ttype" ) continue ;
            std::istringstream si(line);
            si>>id>>type ;
            if( type == "is_unique" ) unique_contigs.insert( id ) ;
        }
    }
