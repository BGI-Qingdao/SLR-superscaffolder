...
struct AppConfig
{
    ...
    std::map<unsigned int, ContigOrientationInfo> gapfills;
    ...
    {
        auto in  = BGIQD::FILES::FileReaderFactory::GenerateReaderFromFileName(fName.orientation());
        if( in == NULL )
            FATAL(" failed to open xxx.orientation for read!!! ");
        auto filler = [this](const std::string & line) ->void
        {
            ContigOrientationInfo fill;
            fill.InitFromString(line);
            gapfills[fill.contig_id] = fill ;
            gapfills[fill.contig_id+1] = fill ;
        };

        BGIQD::FILES::FileReaderFactory::EachLine(*in,filler);
        delete in ;
